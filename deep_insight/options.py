...
def get_opts(fp_hdf_out, train_test_times):
    ...
    # -------- DATA ------------------------
    opts["fp_hdf_out"] = (
        fp_hdf_out  # Filepath for hdf5 file storing wavelets and outputs
    )
    opts["sampling_rate"] = 512 * 4  # Sampling rate of the wavelets
    opts["training_indices"] = train_test_times[
        0
    ]  # Indices into wavelets used for training the model, adjusted during CV
    opts["testing_indices"] = train_test_times[
        1
    ]  # Indices into wavelets used for testing the model, adjusted during CV
    opts["channels"] = 16

    # -------- MODEL PARAMETERS --------------
    opts["model_function"] = "Standard_Decoder"  # Model architecture used
    opts["model_timesteps"] = (
        64  # How many timesteps are used in the input layer, e.g. a sampling rate of 30 will yield 2.13s windows. Has to be divisible X times by 2. X='num_convs_tsr'
    )
    opts["num_convs_tsr"] = (
        5  # Number of downsampling steps within the model, e.g. with model_timesteps=64, it will downsample 64->32->16->8->4 and output 4 timesteps
    )
