...
class PoseCells(object):
    ...
    def __init__(
        ...
        self.excitatory_wrapped_xy = self._get_wrapped_array(
            self.dims[0], self.local_excitation_dimension
        )
        self.excitatory_wrapped_th = self._get_wrapped_array(
            self.dims[2], self.local_excitation_dimension
        )
        self.excitatory_spread_matrix = self._create_pose_cell_spread_matrix(
            self.local_excitation_dimension, self.local_excitation_variance
        )
        self.inhibitory_wrapped_xy = self._get_wrapped_array(
            self.dims[0], self.local_inhibition_dimension
        )
        self.inhibitory_wrapped_th = self._get_wrapped_array(
            self.dims[2], self.local_inhibition_dimension
        )
        self.inhibitory_spread_matrix = self._create_pose_cell_spread_matrix(
            self.local_inhibition_dimension, self.local_inhibition_variance
        )
        ...
        self.activation_center_wrapped_xy = (
            list(
                range(
                    self.dims[0] - self.activation_center_cells_to_average, self.dims[0]
                )
            )
            + list(range(self.dims[0]))
            + list(range(self.activation_center_cells_to_average))
        )
        self.activation_center_wrapped_th = (
            list(
                range(
                    self.dims[2] - self.activation_center_cells_to_average, self.dims[2]
                )
            )
            + list(range(self.dims[2]))
            + list(range(self.activation_center_cells_to_average))
        )
        self.xy_sin_lookup = np.sin(
            np.multiply(range(1, self.dims[0] + 1), (2 * np.pi) / self.dims[0])
        )
        self.xy_cos_lookup = np.cos(
            np.multiply(range(1, self.dims[0] + 1), (2 * np.pi) / self.dims[0])
        )
        self.th_sin_lookup = np.sin(
            np.multiply(range(1, self.dims[2] + 1), (2 * np.pi) / self.dims[2])
        )
        self.th_cos_lookup = np.cos(
            np.multiply(range(1, self.dims[2] + 1), (2 * np.pi) / self.dims[2])
        )
    ...
    def step(self, view_cell, vtrans, vrot):
        ...
        self.active_cell = self._get_center_of_activation(
            self.activation_center_wrapped_xy, self.activation_center_wrapped_th
        )
    ...
    def _inject_energy(self, cell, x, y, th):
        ...
        energy = (
            pc_vt_inject_energy * (1.0 / 30.0) * (30 - np.exp(1.2 * cell.get_decay()))
        )
        if energy > 0:
    ...
    def _update_activation(self):
        ...
        for i, j, k in zip(*indices):
            ...
            new_activations[
                np.ix_(
                    self.excitatory_wrapped_xy[i : i + self.local_excitation_dimension],
        ...
        for i, j, k in zip(*indices):
            ...
            inhibitions[
                np.ix_(
                    self.inhibitory_wrapped_xy[i : i + self.local_inhibition_dimension],
        ...
        self.cells[
            self.cells >= self.global_inhibition_value
        ] -= self.global_inhibition_value
    ...
    def _get_wrapped_array(self, dim, wrap_dimension):
        ...
        """
        return (
            list(range(dim - int(np.floor(wrap_dimension / 2.0)), dim))
            + list(range(dim))
            + list(range(int(np.floor(wrap_dimension / 2.0))))
        )
    ...
    def _get_center_of_activation(self, wrapped_xy, wrapped_th):
        ...
        x = (
            np.arctan2(
                np.sum(self.xy_sin_lookup * x_sums), np.sum(self.xy_cos_lookup * x_sums)
            )
            * self.dims[0]
            / (2 * np.pi)
        ) % (self.dims[0])
        y = (
            np.arctan2(
                np.sum(self.xy_sin_lookup * y_sums), np.sum(self.xy_cos_lookup * y_sums)
            )
            * self.dims[1]
            / (2 * np.pi)
        ) % (self.dims[1])
        th = (
            np.arctan2(
                np.sum(self.th_sin_lookup * th_sums),
    ...
    def _path_integration(self, translation, rotation):
        ...
        direction_bin_size = (2.0 * np.pi) / self.dims[2]
        ...
        for direction_bin_index in range(self.dims[2]):
            ...
            pca90 = np.rot90(
                self.cells[:, :, direction_bin_index],
            ...
            weight_se = translation * np.sin(dir90) - (translation**2) * np.cos(
                dir90
            ) * np.sin(dir90)
            weight_nw = translation * np.cos(dir90) - (translation**2) * np.cos(
                dir90
            ) * np.sin(dir90)
            ...
            self.cells[:, :, direction_bin_index] = np.rot90(
                pca90, 4 - int(np.floor(direction * 2 / np.pi))
            )
        ...
        if rotation != 0:
            ...
            shift1 = int(
                np.sign(rotation) * int(np.floor(abs(rotation) / direction_bin_size))
            )
            shift2 = int(
                np.sign(rotation) * int(np.ceil(abs(rotation) / direction_bin_size))
            )
            self.cells = np.roll(self.cells, shift1, 2) * (1.0 - weight) + np.roll(
                self.cells, shift2, 2
            ) * (weight)
